...
variant_n1 = [1];              %variant_n = [6,6];sectionNum1 =[1,6];%对应孔1的组数sectionNum2 =[1,1];%对应孔2的组数
sectionNum1 =[4];%对应孔1的组数
% sectionNum2 =[4];%对应孔2的组数
% variant_n2 = [4];
% variant_lp2 = [0.16];
%  variant_lv = [5,1.5,0.955,0.5,0.25];
variant_V = [0.01];%0.001时比较接近普通孔管矩阵的计算结果，由于多个亥姆霍兹共鸣器互相影响，必然会有一些等效体积的损失


for i = 1:length(variant_V)
    ...
    para(i).vhpicStruct.lc1 = 0.005;%内插管壁厚,也是孔上插管的长度
    para(i).vhpicStruct.lc2 = 0.2;%内插管壁厚
    para(i).vhpicStruct.lc3 = 0.01;%内插管壁厚
    para(i).vhpicStruct.lc4 = 0.05;%内插管壁厚
    para(i).vhpicStruct.dp = 0.02;%开孔径
    para(i).vhpicStruct.Lin = 0.2;%内插管入口段长度
    para(i).vhpicStruct.lp1 = 0.16;%内插管入口段长度
%     para(i).vhpicStruct.lp2 = variant_lp2(i);%内插管出口段非孔管开孔长度
    para(i).vhpicStruct.n1 = 1;%入口段孔数
%     para(i).vhpicStruct.n2 = variant_n2(i);%出口段孔数
    para(i).vhpicStruct.la1 = 0.01;%孔管入口段靠近入口长度
    para(i).vhpicStruct.la2 = 0.03;%孔管
%     para(i).vhpicStruct.lb1 = 0.03;
%     para(i).vhpicStruct.lb2 = 0.01;
    para(i).vhpicStruct.Din = 0.106;
    para(i).vhpicStruct.Lout = 0.2;
    para(i).vhpicStruct.V = variant_V(i);%(pi.*para(i).vhpicStruct.Dv.^2./4-pi.*para(i).vhpicStruct.Din.^2./4)*para(i).vhpicStruct.Lin;  %0.03
...
%     para(i).vhpicStruct.xSection2
    ...
    if(holepipeLength1 ~= hl1)
        error('孔管参数设置错误：holepipeLength1=%g,hl1=%g;Lin:%g,la1:%g,la2:%g,sum(xSection1):%g,dp:%g'...
            ,holepipeLength1,hl1...
...
%     end
    name{i} = sprintf('V:%g',variant_V(i));
end
...
for i = 1:length(para)
    ...
    [pressure1,pressure2] = ...
        vesselHavePerfInletHelmSeriesNoneOutCalc(massFlowE,Fre,time,...
