function M = vesselHavePerfInletCompOpenTransferMatrix(Dpipe,Dv,l,Lv,...
...
Mv = havePerforatedPipeInletCompOpenTransferMatrix(a,k,Dv,Lv,...
    lc,dp,lp1,lp2,n1,n2,la1,la2,lb1,lb2,Din,xSection1,xSection2,optDamping,optMach);
M = M2 * Mv * M1;
end
function M = havePerforatedPipeInletCompOpenTransferMatrix(a,k,Dv,Lv ...
    ,lc,dp,lp1,lp2,n1,n2,la1,la2,lb1,lb2,Din,xSection1,xSection2,optDamping,optMach)
%缓冲罐内入口连接孔管,开孔个数不足以等效为亥姆霍兹共鸣器。孔管出口部分开口，但仍认为是孔管
%      L1     l                 Lv              l    L2  
%              _________________________________        
...
%     Mca = innerPipeCavityTransferMatrix(Dv,Din,lb1+lp2,'a',a,'k',k);
    ...
    Msplit = innerPerfPipeEndSplitTransferMatrix(Din,Dv,lb2...
        ,'k',k,'a',a,'meanflowvelocity',optDamping.mfvInnerPipe...
        ,'M1',optMach.machInnerPipe,'M2',optMach.machVesselWithInnerPipe);
    %Msplit对结果影响很大，考虑用内插管腔体矩阵来代替，即内插管腔体传递矩阵-右边
...
%     M = Mv1 * Msplit * Mp2 * Mstr2 * Mp1 * Mstr1;
    ...
    M = Mv1 * Mstr3 * Mp2 * Mstr2 * Mp1 * Mstr1;    
end