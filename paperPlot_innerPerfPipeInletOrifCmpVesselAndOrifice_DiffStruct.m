...
for i = 1:length(para)
    ...
    [pressure1OrificeP,pressure2OrificeP] = ...
        ...
        'meanFlowVelocity',para(i).opt.meanFlowVelocity,'isUseStaightPipe',1,...
        'm',para(i).opt.mach,'notMach',para(i).opt.notMach);%,'coeffDamping',opt.coeffDamping
        plus1Orifice{i} = calcPuls(pressure1OrificeP,dcpss);
        plus2Orifice{i} = calcPuls(pressure2OrificeP,dcpss);
        plusOrifice{i} = [plus1Orifice{i},plus2Orifice{i}];
        namePlusOrifice{i} = sprintf('孔管：n:%g',inputData(i,10));
%    %计算带孔板缓冲罐
%     if i == 1
%         [pressure1OrificeP,pressure2OrificeP] = ...
...
%     end
    ...
    Y=Y';
    h(plotCount) = plot(X,Y,marker_style{marker_index},'color',color_style(color_index,:),'LineWidth',1.5);  
    textLegend{plotCount} = namePlusOrifice{i};
    plotCount = plotCount + 1;
end
ax = axis;
