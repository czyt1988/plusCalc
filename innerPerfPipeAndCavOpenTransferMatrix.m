function M = innerPerfPipeAndCavOpenTransferMatrix(n2,dp,Din,Dv,lp2,lc,lb1,lb2,xSection2,varargin)
%内插孔管出口部分，孔管出口处开放，缓冲罐全开,理论自推
% n2 孔管穿孔数
% dp 孔管穿孔孔径
% Din 穿管管径
% lp2 穿孔部分长度
...
function Tmn = calcTmn(m,n,k,lb1,lp2,R)
%lb1 靠近入口无穿孔距离
%lb2 靠近出口无穿孔距离
    E = -1i.*tan(k.*lb1);
    F = -1i.*tan(k.*(lb1+lp2));
