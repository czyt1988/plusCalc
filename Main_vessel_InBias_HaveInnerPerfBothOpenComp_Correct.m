...
for i = 1:length(para)
    ...
    [pressure1BOpenIB,pressure2BOpenIB] = ...
        vesselIBHaveInnerPerfBothOpenCompCalc(massFlowE,Fre,time,...
    ...
    plus1BOpenIB{i} = calcPuls(pressure1BOpenIB,dcpss);
    plus2BOpenIB{i} = calcPuls(pressure2BOpenIB,dcpss);
    plusBOpenIB{i} = [plus1BOpenIB{i},plus2BOpenIB{i}];
    multFreAmpValueBOpenIB{i} = calcWaveFreAmplitude([pressure1BOpenIB,pressure2BOpenIB],Fs,multFre,'freErr',1);
    
    calcDatas{dataCount,1} = sprintf('入口偏置内插孔管两端开口缓冲罐,n1:%g,n2:%g',variant_n1(i),variant_n2(i));
    calcDatas{dataCount,2} = X;
    calcDatas{dataCount,3} = plusBOpenIB{i};
    calcDatas{dataCount,4} = multFreAmpValueBOpenIB{i}(1,:);
    calcDatas{dataCount,5} = multFreAmpValueBOpenIB{i}(2,:);
    calcDatas{dataCount,6} = multFreAmpValueBOpenIB{i}(3,:);
    dataCount = dataCount + 1;
    ...
    plus2BOpenIB{i} = calcPuls(pressure2OpenIB,dcpss);
    plusBOpenIB{i} = [plus1OpenIB{i},plus2BOpenIB{i}];
    multFreAmpValueOpendIB{i} = calcWaveFreAmplitude([pressure1OpenIB,pressure2OpenIB],Fs,multFre,'freErr',1);
    calcDatas{dataCount,1} = sprintf('入口偏置内插孔管出口开口缓冲罐,n1:%g,n2:%g',variant_n1(i),variant_n2(i));
    calcDatas{dataCount,2} = X;
    calcDatas{dataCount,3} = plusBOpenIB{i};
    ...
    if isempty(plus1OpenIB{i})
        maxPlus1(i) = nan;
    else
        maxPlus1(i) = max(plus1OpenIB{i});
    end

    if isempty(plus2BOpenIB{i})
        maxPlus2(i) = nan;
    else
        maxPlus2(i) = max(plus2BOpenIB{i});
    end  
