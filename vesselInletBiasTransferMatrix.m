function [M,k] = vesselInletBiasTransferMatrix(Lv,l,lv1,lv2,Dbias1,Dbias2,varargin )
%缓冲罐错位的气流脉动计算
% Dbias 偏置管内插入缓冲罐的管径，如果偏置管没有内插如缓冲罐，Dbias为0,角标1为入口
...
Mv = vesselMatrix(isUseStaightPipe,Lv,lv1,lv2,k,Dvessel,Dbias1,Dbias2,a,optDamp,optMachVessel);
%后管道传递矩阵
...
function Mv = vesselMatrix(isUseStaightPipe,Lv,lv1,lv2,k,Dv,Dbias1,Dbias2,a,optDamping,optMach)
    ...
    if isUseStaightPipe%使用直管理论
        ...
        %内插管腔体传递矩阵-左边
        innerRM = innerPipeCavityTransferMatrix(Dv,Dbias2,lv2,'a',a,'k',k);
        %内插管腔体传递矩阵-右边
        innerLM = innerPipeCavityTransferMatrix(Dv,Dbias1,lv1,'a',a,'k',k);

         Mv = innerRM*ML*innerLM;
        return;
    end
%     %使用容积传递矩阵
%     V = Sv*L;
