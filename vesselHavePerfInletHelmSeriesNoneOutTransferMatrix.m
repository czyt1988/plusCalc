function M = vesselHavePerfInletHelmSeriesNoneOutTransferMatrix(Dpipe,Dv,l,Lv,...
    lc1,lc2,lc3,lc4,dp,lp1,n1,la1,la2,Din,Lout,V,lv,varargin)
%缓冲罐内入口连接孔管,Lin部分等效为串联的亥姆霍兹共鸣器.Lout部分为内插开口管，不开孔
%      L1     l                 Lv              l    L2  
%              _________________________________        
%             | lc dp(n1) |                     |
%             |___ _ _ ___|___________          |     
%  -----------|___________ ___________ Din      |----------
%             |la1 lp1 la2|                     |
...
Mv = havePerforatedPipeInletHelmSeriesNoneOutTransferMatrix(a,k,Dv,Lv,...
    lc1,lc2,lc3,lc4,dp,lp1,n1,la1,la2,Din,Lout,V,lv,optDamping,optMach);
M = M2 * Mv * M1;
end
function M = havePerforatedPipeInletHelmSeriesNoneOutTransferMatrix(a,k,Dv,Lv ...
    ,lc1,lc2,lc3,lc4,dp,lp1,n1,la1,la2,Din,Lout,V,lv,optDamping,optMach)
%缓冲罐内入口连接孔管,开孔个数不足以等效为亥姆霍兹共鸣器
%      L1     l                 Lv              l    L2  
%              _________________________________        
%             |    dp(n1) |                     |
...
% lp1 孔管入口段开孔长度
% Din 孔管管径；
% xSection1孔管每圈孔的间距，从0开始算，x的长度为孔管孔的圈数+1，x的值是当前一圈孔和上一圈孔的距离，如果间距一样，那么x里的值都一样
   ...
   %内插孔管入口段对应lp1，等效为串联的n个亥姆霍兹共鸣器
    HM1 = HelmholtzResonatorTransferMatrix_nInSeries(V,lv,lc1,dp...
        ,'k',k,'a',a);
    HM2 = HelmholtzResonatorTransferMatrix_nInSeries(V,lv,lc2,dp...
        ,'k',k,'a',a);
    HM3 = HelmholtzResonatorTransferMatrix_nInSeries(V,lv,lc3,dp...
        ,'k',k,'a',a);
    HM4 = HelmholtzResonatorTransferMatrix_nInSeries(V,lv,lc4,dp...
        ,'k',k,'a',a);
    %内插孔管与进口管连接部分对应la1
    ...
    %内插孔管沿轴线相邻两个孔内边缘间距
    l = lp1./n1;
    Mv = straightPipeTransferMatrix(l,'k',k,'d',Dv,'a',a,...
                'isDamping',optDamping.isDamping,'coeffDamping',optDamping.coeffDampVessel...
                ,'mach',optMach.machVessel,'notmach',optMach.notMach);
    M = Mv1 * innerRM * Mstr2 * HM4 * Mv * HM3 * Mv * HM2 * Mv * HM1 * Mstr1;%假定有四个孔，即四个亥姆霍兹共鸣器串联
end