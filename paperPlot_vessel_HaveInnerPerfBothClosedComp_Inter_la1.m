...
variant_lp1 = [0.07];
variant_lp2 = [0.16];
variant_la2 = [0.02;0.06;0.12;0.18;0.24;0.3;0.36];%0.01;0.06;0.11;0.16;0.21;0.26;0.31
variant_lb1 = [0.06;0.06;0.06;0.06;0.06;0.06;0.06];
% variant_Lv1 = 0.568:0.02:0.84;
calcDatas = {};


for i = 1:length(variant_la2)     
    para(i).opt = opt;
    para(i).L1 = 3.5;%L1(m)
    para(i).L2 = 6;%L2（m）长度
    ...
    para(i).vhpicStruct.Lv1 = 0.66;%0.84;%para(i).vhpicStruct.Lv./2;%缓冲罐腔1总长
    ...
    para(i).vhpicStruct.Lin = 0.45;%内插管入口段长度
    para(i).vhpicStruct.Lout = 0.25;
    para(i).vhpicStruct.lp1 = variant_lp1;%内插管入口段非孔管开孔长度
    para(i).vhpicStruct.lp2 = variant_lp2;%内插管出口段孔管开孔长度
    ...
    para(i).vhpicStruct.la1 = para(i).vhpicStruct.Lin-variant_lp1-variant_la2(i);
    para(i).vhpicStruct.lb2 = 0.03;%para(i).vhpicStruct.Lout-variant_lb1(i)-variant_lp2;
    %para(i).vhpicStruct.Lin = variant_la2(i)+variant_lp1+para(i).vhpicStruct.la1;
    ...
    para(i).vhpicStruct.s1 = ((variant_lp1./para(i).vhpicStruct.Cloum1)-variant_dp1)./2;%相邻两开孔之间间隔，默认等间隔
    para(i).vhpicStruct.s2 = ((variant_lp2./para(i).vhpicStruct.Cloum2)-variant_dp2)./2;
    para(i).vhpicStruct.sc1 = (pi.*variant_Din - para(i).vhpicStruct.nc1.*para(i).vhpicStruct.dp1)./para(i).vhpicStruct.nc1;%一周开孔，相邻孔间距
    para(i).vhpicStruct.sc2 = (pi.*variant_Din - para(i).vhpicStruct.nc2.*para(i).vhpicStruct.dp2)./para(i).vhpicStruct.nc2;
    ...
    name{i} = sprintf('la2:%g',variant_la2(i));
end
...
for i = 1:length(para)
    ...
    calcDatas{dataCount,1} = sprintf('入口内插孔管两端堵死缓冲罐,la2:%g',variant_la2(i));
    calcDatas{dataCount,2} = X;
...
%     dataCount = dataCount + 1;
    ...
    if i == 1
        ...
        [pressure1Temp,pressure2Temp] = oneVesselPulsationCalc(massFlowE,Fre,time,...
            para(i).L1,para(i).L2,...
        ...
        calcDatas{dataCount,1} = sprintf('无内件缓冲罐');
	    calcDatas{dataCount,2} = X;
