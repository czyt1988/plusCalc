function M = innerPerfPipeDoubOpenLeftTransferMatrix(n1,dp,Din,Dv,lp1,lc,la1,la2,varargin)
%对应孔管内部部分
...
[Ta9,Tb9,Tc9,Td9] = calcT1(n1,dp,Din,Dv,la1,la2,lp1,k,M1,M2,R0,lc);
...
function [Ta9,Tb9,Tc9,Td9] = calcT1(n1,dp,Din,Dv,la1,la2,lp1,k,M1,M2,R0,lc)
% n1 孔管穿孔数
% dp 孔管穿孔孔径
% Din 穿管管径
% lp1 穿孔部分长度
...
% lc 孔管壁厚
    ...
    [C31,C32,C41,C42] = calcCoeffC(k,la2,lp1,AperB,CperD,Fai,Fi,beita);
...
function CperD = calcCoeffCperD(k,areaRatio1,areaRatio2,M2,la1)
    CperD = k.*(2.*(1-exp(1i.*k.*la1.*(areaRatio2./areaRatio1).*M2)) - cos(k.*la1).*(cos(k.*la1) + 1i.*(1-1./areaRatio2)...
        ...
        -1i.*(1-1./areaRatio2).*(areaRatio2./areaRatio).*M2.*cos(k.*la1)));
end
