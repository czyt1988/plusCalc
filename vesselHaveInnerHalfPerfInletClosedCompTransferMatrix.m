function M = vesselHaveInnerHalfPerfInletClosedCompTransferMatrix(Dpipe,Dv,l,Lv,...
    lc,dp,lp1,n1,la1,la2,Din,xSection1,varargin)
%缓冲罐中间插入孔管,末端堵死，开孔个数不足以等效为亥姆霍兹共鸣器
%      L1     l                 Lv              l    L2  
...
Mv = haveInnerHalfPerfPipeInClosedCompTransferMatrix(a,k,Dv,Dv_inner,Lv,...
    lc,dp,lp1,n1,la1,la2,Din,xSection1,optDamping,optMach);
M = M2 * Mv * M1 ;

end
function M = haveInnerHalfPerfPipeInClosedCompTransferMatrix(a,k,Dv,Dv_inner,Lv ...
    ,lc,dp,lp1,n1,la1,la2,Din,xSection1,optDamping,optMach)
%缓冲罐中间插入孔管,末端堵死，开孔个数不足以等效为亥姆霍兹共鸣器
%      L1     l                 Lv              l    L2  
%              _________________________________        
...
%     Lv2 = Lv/2 - Lout;%缓冲罐内插孔管无交接的区域长
    ...
    if ((Lv1 < 0))
     ...
     Mv2 = straightPipeTransferMatrix(Lv/2,'k',k,'d',Dv,'a',a,...
...
%         ,'M1',optMach.machInnerPipe,'M2',optMach.machVesselWithInnerPipe);
    %内插孔管隔板区对应la2
    Mstr = straightPipeTransferMatrix(la2,'k',k,'d',Din,'a',a,...
