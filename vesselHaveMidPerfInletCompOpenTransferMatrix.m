function M = vesselHaveMidPerfInletCompOpenTransferMatrix(Dpipe,Dv,l,Lv,...
    lc,dp,lp1,lp2,n1,n2,la1,la2,lb1,lb2,Din,Lv1,xSection1,xSection2,varargin)
%缓冲罐内孔管,开孔个数不足以等效为亥姆霍兹共鸣器.孔管进、出口部分开口，但仍认为是孔管
%      L1     l                 Lv                 l    L2  
...
Mv = haveMidPerforatedPipeInletCompOpenTransferMatrix(a,k,Dv,Lv,...
    lc,dp,lp1,lp2,n1,n2,la1,la2,lb1,lb2,Din,Lv1,xSection1,xSection2,optDamping,optMach);
M = M2 * Mv * M1;
end
function M = haveMidPerforatedPipeInletCompOpenTransferMatrix(a,k,Dv,Lv ...
    ,lc,dp,lp1,lp2,n1,n2,la1,la2,lb1,lb2,Din,Lv1,xSection1,xSection2,optDamping,optMach)
%缓冲罐内入口连接孔管,开孔个数不足以等效为亥姆霍兹共鸣器.孔管出口部分开口，但仍认为是孔管
%      L1     l                 Lv                 l    L2  
%              ____________________________________        
%             |    dp(n1)    |    dp(n2)           |
%             |   ___ _ _ ___|___ _ _ ___ lc       |     
...
% xSection1，xSection2 孔管每圈孔的间距，从0开始算，x的长度为孔管孔的圈数+1，x的值是当前一圈孔和上一圈孔的距离，如果间距一样，那么x里的值都一样
   ...
   if ~isstruct(optDamping)
    ...
    Lin = la1 + lp1 + la2;
    Lout = lb1 + lp2 + lb2;
    Lv2 = Lv - Lin - Lout-Lv1;%缓冲罐内插孔管无交接的区域长
...
%                 ,'mach',optMach.machVesselWithInnerPipe,'notmach',optMach.notMach);
    %内插孔管出口段对应lp2
    Mp2 = innerPerfPipeAndCavOpenTransferMatrix(n2,dp,Din,Dv,lp2,lc,lb1,lb2,xSection2...
    ...
    %内插孔管入口段对应lp1
    Mp1 = innerMidPerfPipeAndCavOpenTransferMatrix(n1,dp,Din,Dv,lp1,lc,la1,la2,xSection1...
        ,'k',k,'a',a,'meanflowvelocity',optDamping.mfvInnerPipe...
        ,'M1',optMach.machInnerPipe,'M2',optMach.machVesselWithInnerPipe);
    %内插孔管与进口管连接部分对应la1
...
%     Mca = innerPipeCavityTransferMatrix(Dv,Din,lb1+lp2,'a',a,'k',k);
    %内插孔管出口部分末端对应lb2(暂时利用分流回流矩阵)

    Mstr3 = straightPipeTransferMatrix(lb2,'k',k,'d',Din,'a',a,...
                'isDamping',optDamping.isDamping,'coeffDamping',optDamping.coeffDampInnerPipe...
                ,'mach',optMach.machInnerPipe,'notmach',optMach.notMach);
%     M = Mv1 * Msplit * Mp2 * Mstr2 * Mp1 * Mstr1;

    M = Mv2 * Mstr3 * Mp2 * Mstr2 * Mp1 * Mstr1 * Mv1;    
end