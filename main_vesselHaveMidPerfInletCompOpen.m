%% 缓冲罐内置孔管结构与单一顺接缓冲罐对比
%缓冲罐内置孔管,开孔个数不足以等效为亥姆霍兹共鸣器.孔管进、出口部分开口，但仍认为是孔管
clc;
close all;
clear;
...
sectionNum2 =[4,4,4,4,4];%对应孔2的组数
sectionNum1 =[4,4,4,4,4];%对应孔1的组数,要看是否保证lp1不变
variant_dp = [0.001,0.005,0.01,0.02,0.03];
...
for i = 1:length(variant_dp)
    ...
    para(i).vhpicStruct.bp1 = 0.0825;
    para(i).vhpicStruct.lp1 = 0.16;%内插管入口段非孔管开孔长度
    para(i).vhpicStruct.Lv1 = 0.35;%孔管进口段端部距离缓冲罐入口的距离
    para(i).vhpicStruct.Lin = 0.2;%内插管入口段长度
    para(i).vhpicStruct.n1 = (4.*para(i).vhpicStruct.Din.*para(i).vhpicStruct.lp1.*para(i).vhpicStruct.bp1)./((para(i).vhpicStruct.dp)^2);%入口段孔数
    para(i).vhpicStruct.lp2 = 0.16;%内插管出口段非孔管开孔长度
    para(i).vhpicStruct.n2 = 28;%出口段孔数
...
for i = 1:length(para)
    ...
    [pressure1,pressure2] = ...
        vesselHaveMidPerfInletCompOpenCalc(massFlowE,Fre,time,...
