...
temp = [1:50];%(Fre<29) ;%| (Fre>30 & Fre < 100);
...
for i = 1:length(iteData)     
    ...
    para(i).vhpicStruct.dp = 0.13;%开孔径
    para(i).vhpicStruct.Lin = 0.25;%内插管入口段长度
    para(i).vhpicStruct.lp2 = 0.16;%内插管入口段非孔管开孔长度
    para(i).vhpicStruct.lp1 = 0.16;%内插管出口段孔管开孔长度
    para(i).vhpicStruct.n2 = 48;%入口段孔数
    para(i).vhpicStruct.n1 = 48;%出口段孔数
    para(i).vhpicStruct.lb2 = iteData(i);%孔管入口段靠近入口长度
    ...
    para(i).vhpicStruct.la2 = para(i).vhpicStruct.Lin - para(i).vhpicStruct.la1 - para(i).vhpicStruct.lp1;%孔管;
...
%     reduceRate{i} = (temp - temp2)./temp;
    ...
    if isempty(plus2OpenIB{i})
        maxPlus2(i) = nan;
    else
        maxPlus2(i) = max(plus2OpenIB{i});
    end  
...
figure
plot(iteData,maxPlus2.*70./1000,'LineWidth',1.5);
xlabel('孔管非开孔段长度(m)');
