function M = vesselHavePerfInletCompOpenHelmParaTransferMatrix(Dpipe,Dv,l,Lv,...
    lc,dp,Lin,lp2,n1,n2,la1,la2,lb1,lb2,Din,V,lv,xSection2,varargin)
%缓冲罐内入口连接孔管,入口段等效为亥姆霍兹共鸣器，开孔少，沿周向分布.孔管出口部分开口，但仍认为是孔管
%      L1     l                 Lv              l    L2  
...
Mv = havePerforatedPipeInletCompOpenHelmParaTransferMatrix(a,k,Dv,Lv,...
    lc,dp,Lin,lp2,n1,n2,la1,la2,lb1,lb2,Din,V,lv,xSection2,optDamping,optMach);
M = M2 * Mv * M1;
end
function M = havePerforatedPipeInletCompOpenHelmParaTransferMatrix(a,k,Dv,Lv ...
    ,lc,dp,Lin,lp2,n1,n2,la1,la2,lb1,lb2,Din,V,lv,xSection2,optDamping,optMach)
%缓冲罐内入口连接孔管,入口段等效为亥姆霍兹共鸣器，开孔少，沿周向分布。孔管出口部分开口，但仍认为是孔管
%      L1     l                 Lv              l    L2  
%              _________________________________        
...
%     Mca = innerPipeCavityTransferMatrix(Dv,Din,lb1+lp2,'a',a,'k',k);
    ...
    Msplit = innerPerfPipeEndSplitTransferMatrix(Din,Dv,lb2...
        ,'k',k,'a',a,'meanflowvelocity',optDamping.mfvInnerPipe...
        ,'M1',optMach.machInnerPipe,'M2',optMach.machVesselWithInnerPipe);
    %目前不会处理腔体对应的lb2部分，所以暂时忽略
    %
    M = Mv1 * Msplit * Mp2 * Mstr2 * MH1 * Mstr1;
end