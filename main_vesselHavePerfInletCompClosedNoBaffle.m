%% 缓冲罐内置孔管结构与单一顺接缓冲罐对比
%缓冲罐内入口连接孔管,开孔个数不足以等效为亥姆霍兹共鸣器.孔管出口部分闭口，不加隔板，验证西交大论文
clc;
close all;
clear;
...
sectionNum2 =[10000];%对应孔2的组数
variant_n2 = [56];

for i = 1:length(variant_n2)
    ...
    para(i).vhpicStruct.n2 = variant_n2(i); %(4.*para(i).vhpicStruct.Din.*para(i).vhpicStruct.lp2.*para(i).vhpicStruct.bp2)./((para(i).vhpicStruct.dp)^2);%入口段孔数
%     para(i).vhpicStruct.Lin = 0.2;%内插管入口段长度
    para(i).vhpicStruct.lb1 = 0.01;
    para(i).vhpicStruct.lb2 = 0.01;%para(i).vhpicStruct.Lin-para(i).vhpicStruct.lb1-para(i).vhpicStruct.lp2;%假定保证入口段la1始终不变，0.15
    para(i).vhpicStruct.lp2 = 0.4-para(i).vhpicStruct.lb1-para(i).vhpicStruct.lb2;%内插管出口段非孔管开孔长度
...
%     holepipeLength1 = para(i).vhpicStruct.Lin - para(i).vhpicStruct.lb1 - para(i).vhpicStruct.lb2;
%     hl1 = sum(para(i).vhpicStruct.xSection2);
%     if(~cmpfloat(holepipeLength1,hl1))
%         error('孔管参数设置错误：holepipeLength1=%.8f,hl1=%.8f;Lin:%g,la1:%g,la2:%g,sum(xSection1):%g,dp:%g'...
%             ,holepipeLength1,hl1...
%             ,para(i).vhpicStruct.Lin,para(i).vhpicStruct.lb1,para(i).vhpicStruct.lb2...
%             ,sum(para(i).vhpicStruct.xSection2),para(i).vhpicStruct.dp);
%     end
   
    name{i} = sprintf('n2:%g',variant_n2(i));
end
...
for i = 1:length(para)
    ...
    [pressure1,pressure2] = ...
        vesselHavePerfInletCompNoBaffleCompCalc(massFlowE,Fre,time,...
...
%     %计算缓冲罐内置孔管隔板
%     if i == 1
%         [pressure1PB,pressure2PB] = vesselHavePerfInletNoneOutCalc(massFlowE,Fre,time,...
%             para(i).L1,para(i).L2,para(i).Dpipe...
%         ,para(i).vhpicStruct,...
%         para(i).sectionL1,para(i).sectionL2,...
%         'a',para(i).opt.acousticVelocity,'isDamping',para(i).opt.isDamping,'friction',para(i).opt.coeffFriction,...
%         'meanFlowVelocity',para(i).opt.meanFlowVelocity,...
%         'm',para(i).opt.mach,'notMach',para(i).opt.notMach);
%         plus1PB = calcPuls(pressure1PB,dcpss);
%         plus2PB = calcPuls(pressure2PB,dcpss);
%         plusPB = [plus1PB,plus2PB];
%     end
    %计算单一缓冲罐
    if i == 1
