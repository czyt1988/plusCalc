...
inputData = [...
    ...
    %L1 , L2, Dpipe,Dv ,l  , Lv   ,lc1    ,lc2,  lv1  ,lv2,  dp1 , dp2,   n1,  
    %n2,  Lin,Lout, Li, Lo,  V1 ,  V2,     Din1,  Din2
    3.35, 10, 0.106,0.45,0.01,1.18,0.005,  0.005,0.591,0.591,0.02,0.013,  28,...
    68,  0.2, 0.2, 0.2, 0.2, 0.0938,0.0938,0.106, 0.106/2
...
for i = 1:size(inputData,1)
    name{i} = sprintf('n1:%g,n2:%g',inputData(i,13),inputData(i,14));
    ...
    para(i).lc1 = inputData(i,7);%
    para(i).lc2 = inputData(i,8);
    para(i).lv1 = inputData(i,9);%
    para(i).lv2 = inputData(i,10);
    para(i).dp1 = inputData(i,11);%
    para(i).dp2 = inputData(i,12);
    para(i).n1 = inputData(i,13);
    para(i).n2 = inputData(i,14);%
    para(i).Lin = inputData(i,15);%
    para(i).Lout = inputData(i,16);
    para(i).Li = inputData(i,17);%
    para(i).Lo = inputData(i,18);
    para(i).V1 = inputData(i,19);
    para(i).V2 = inputData(i,20);
    para(i).Din1 = inputData(i,21);
    para(i).Din2 = inputData(i,22);
...
for i = 1:length(para)
    ...
    [pressure1,pressure2] = ...
        inDoubPerfPipeVesselPulsationCalc(massFlowE,Fre,time,...
        para(i).L1,para(i).L2,...
        para(i).Dpipe,para(i).Dv,para(i).l,para(i).Lv,...
        para(i).lc1,para(i).lc2,para(i).lv1,para(i).lv2,para(i).dp1,para(i).dp2,...
        para(i).n1,para(i).n2,para(i).Lin,para(i).Lout,para(i).Li,para(i).Lo,...
        para(i).V1,para(i).V2,para(i).Din1,para(i).Din2,...
    ...
    if i == 1
        ...
        para(i).L1,para(i).L2,...
        para(i).Dpipe,para(i).Dv,para(i).l,para(i).Lv,...
        para(i).lc1,para(i).lv1,para(i).dp1,para(i).n1,...
        para(i).Lin,para(i).Lout,para(i).V1,para(i).Din1,...
        para(i).sectionL1,para(i).sectionL2,...
        'a',para(i).opt.acousticVelocity,'isDamping',para(i).opt.isDamping,'friction',para(i).opt.coeffFriction,...
        'meanFlowVelocity',para(i).opt.meanFlowVelocity,'isUseStaightPipe',1,...
        'm',para(i).opt.mach,'notMach',para(i).opt.notMach);%,'coeffDamping',opt.coeffDamping
...
%     end
     if i==1
        ...
        textLegend{plotCount} = '缓冲罐入口接孔管n1=28';
        plotCount = plotCount + 1;
     end
%      if i==2
%         %显示入口接孔管
