function M = vesselHaveHalfInnerPerfOutClosedCompTransferMatrix(Dpipe,Dv,l,Lv,...
    lc,dp,lp2,n2,lb1,lb2,Din,xSection2,varargin)
%缓冲罐中间插入孔管,仅出口末端堵死，开孔个数不足以等效为亥姆霍兹共鸣器
%      L1     l                 Lv              l    L2  
...
Mv = haveInnerHalfPerfPipeOutClosedCompTransferMatrix(a,k,Dv,Dv_inner,Lv,...
    lc,dp,lp2,n2,lb1,lb2,Din,xSection2,optDamping,optMach);
M = M2 * Mv * M1 ;

end
function M = haveInnerHalfPerfPipeOutClosedCompTransferMatrix(a,k,Dv,Dv_inner,Lv ...
    ,lc,dp,lp2,n2,lb1,lb2,Din,xSection2,optDamping,optMach)
%缓冲罐中间插入孔管,仅出口末端堵死，开孔个数不足以等效为亥姆霍兹共鸣器
%      L1     l                 Lv              l    L2  
%              _________________________________        
...
%     end
    ...
    %内插孔管隔板区对应lb1
    Mstr = straightPipeTransferMatrix(lb1,'k',k,'d',Din,'a',a,...
...
%                 ,'mach',optMach.machInnerPipe,'notmach',optMach.notMach);
    %入口孔管前端开放空腔对应
    Mv1 = straightPipeTransferMatrix(Lv/2,'k',k,'d',Dv,'a',a,...
                'isDamping',optDamping.isDamping,'coeffDamping',optDamping.coeffDampVessel...
                ,'mach',optMach.machVessel,'notmach',optMach.notMach);
