function M = vesselHaveInnerPipeTransferMatrix(Lv,l,Dv,Dpipe,Linner ...
...
Mv = haveInnerPipeTransferMatrix(a,k,Lv,Dv,Dpipe,Linner ...
    ,Lin,Lout,Din,optDamping,optMachVessel,mfvVessel);
M = M2 * Mv * M1;
end
%这里都是用直管等效
function M = haveInnerPipeTransferMatrix(a,k,Lv,Dv,Dpipe,Linner ...
    ,Lin,Lout,Din,optDamping,optMach,mfvVessel)
%  长度 L1     l    Lv      l    L2  
%              ______________        
%             |     _|_  Lout|      
%  -----------| Lin _ _Din   |----------
...
%section        1          2   缓冲罐分的两个区域
    ...
    Lv1 = Linner - Lin;%进口缓冲罐于内插管无交接的区域长度
    Lv2 = Lv - Lv1 - Lin - Lout;%缓冲罐区域2和内插管无交接的区域长度
    if ((Lv1 < 0) || (Lv2 < 0))
        error('长度尺寸有误');
    end
    ...
    %内插管的管道传递矩阵
    innerPipeDampingCoeff = Dv^3 / Din^3 * optDamping.coeffDamping;%阻尼系数的传递
